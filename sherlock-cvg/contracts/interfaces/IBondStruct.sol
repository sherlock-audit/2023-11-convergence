...
interface IBondStruct {
    struct BondParams {
        ...
        address token;
        uint32 gamma; // Min bond ROI, divide by 1000 to get the roi in %
        uint48 bondDuration;
        uint32 scale; // Max bond ROI, divide by 1000 to get the roi in %
        uint32 minRoi; // Min bond ROI, divide by 1000 to get the roi in %
        uint32 maxRoi; // Max bond ROI, divide by 1000 to get the roi in %
        uint88 percentageMaxCvgToMint; // Percentage maximum of the maxCvgToMint that an user can mint in one deposit
        uint72 vestingTerm; // In timestamp
