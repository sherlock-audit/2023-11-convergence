...
contract MockCvgUtilities is Ownable2Step {
    ...
    function claimMultipleLocking(ClaimTokenTde[] calldata claimTdes) external {
        ...
        for (uint256 i; i < claimTdes.length; ) {
            ...
            for (uint256 j; j < claimTdes[i].tdeIds.length; ) {
                uint256 tdeId = claimTdes[i].tdeIds[j];

                ysDistributor.claimRewards(tokenId, tdeId, msg.sender, msg.sender);
                unchecked {
                    ++j;
                }
            }
        ...
        if (_lockTokenId > 0 || _lockDuration > 0) {
            ...
            if (_lockTokenId > 0) {
                ...
                if (_durationAdd > 0) {
                    ...
                    _lockingPositionService.increaseLockTimeAndAmount(_lockTokenId, _durationAdd, cvgAmount, operator);
                } else {
                    /// @dev increase locking position amount
                    require(actualCycle + MINIMUM_LOCK_DURATION <= endCycle, "REMAINING_LOCK_DURATION_TOO_LOW");
                    _lockingPositionService.increaseLockAmount(_lockTokenId, cvgAmount, operator);
                }
            } else {
